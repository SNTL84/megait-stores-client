# megait-stores-client

🖥️ **MEGA IT STORES** — Full Client Digital Package  
Built by **Milan · SNTL 84** · [desidevloper.com](https://desidevloper.com)

---

## 📂 File Manifest

| File | Description | Size |
|------|-------------|------|
| `megait-catalog.html` | Full product catalog — CPUs, GPUs, Motherboards, Pre-Builts, PC Builder, Cart, About, Contact | ~130KB |
| `megait-performance-marketing-proposal.html` | Performance Marketing Proposal — Facebook/Instagram setup, 30-day campaign, pricing ₹3,500–₹6,500 | ~54KB |

---

## 📌 Naming Convention

Follows `[client-slug]-[project-type]` pattern:  
`megait-stores-client` → Client: MEGA IT STORES | Type: Client Package

---

## 🔗 Contact

- Website: [desidevloper.com](https://desidevloper.com)
- WhatsApp: [+91 97274 13309](https://wa.me/919727413309)
- LinkedIn: [linkedin.com/in/sntl2784](https://www.linkedin.com/in/sntl2784)
- GitHub: [github.com/SNTL84](https://github.com/SNTL84)

---

> 🔒 Private repo — Client confidential. Do not share.
